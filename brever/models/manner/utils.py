...
def write_result(estimate, noise, file, args):
    ...
    file_name     = opj(args.enhanced_path, file[0].rsplit('.',1)[0].replace('\\','/').split('/')[-1])
...
def args_dict(args):
    ...
    args.setting = {'sample_rate':args.sample_rate, 'segment':args.segment,
                    'pad':args.pad, 'stride':args.set_stride}
    args.manner  = {'in_channels':args.in_channels, 'out_channels':args.out_channels,
                    'hidden':args.hidden, 'depth':args.depth, 'kernel_size':args.kernel_size,
                    'stride':args.stride, 'growth':args.growth, 'head':args.head,
